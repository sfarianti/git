                ...
                <form action="{{ route('assessment.submitJuri', ['id' => Request::segments()[2]]) }}" method="post">
                    ...
                    <div class="card-body">
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <textarea name="recommendation" id="inputRecomCategory" cols="30" rows="3" class="form-control" {{ auth()->user()->role === 'Superadmin' || auth()->user()->role === 'Admin' ? 'disabled' : '' }} require>{{ $sofiData->recommend_category }}</textarea>
                        </div>
                        <div class="col-md-12 mb-3">
                            <label class="small mb-1" for="inputStrength">Strength</label>
                            <textarea name="sofi_strength" id="inputStrength" cols="30" rows="3" class="form-control" {{ auth()->user()->role === 'Superadmin' || auth()->user()->role === 'Admin' ? 'disabled' : '' }} require>{{$sofiData->strength}}</textarea>
                        </div>
                        <div class="col-md-12 mb-3">
                            <label class="small mb-1" for="inputOpportunity">Opportunity</label>
                            <textarea name="sofi_opportunity" id="inputOpportunity" class="form-control" cols="30" rows="3" {{ auth()->user()->role === 'Superadmin' || auth()->user()->role === 'Admin' ? 'disabled' : '' }} require>{{$sofiData->opportunity_for_improvement}}</textarea>
                        </div>
                        <div class="col-md-12 mb-3">
                            <label class="small mb-1" for="inputCommentBenefit">Komentar Benefit</label>
                            <textarea name="suggestion_for_benefit" id="inputCommentBenefit" class="form-control" cols="30" rows="3" {{ auth()->user()->role === 'Superadmin' || auth()->user()->role === 'Admin' ? 'disabled' : '' }} require>{{$sofiData->suggestion_for_benefit}}</textarea>
                        </div>
                    </div>
...
<script type="">
    ...
    function updateColumnDataTable() {
        newColumn = []
        $.ajax({
            url: "{{ route('query.get_input_oda_assessment_team') }}", // Misalnya, URL untuk mengambil kolom yang dinamis
            method: 'GET',
            // dataType: 'json',
            data:{
