...
document.addEventListener("DOMContentLoaded", function () {
    const exportExcelButton = document.querySelector('.export-excel-totalBenefitPotentialCompanyChart');
    const exportPdfButton = document.querySelector('.export-pdf-totalBenefitPotentialCompanyChart');

    if (exportExcelButton) {
        exportExcelButton.addEventListener('click', async function () {
            const chartCanvas = document.getElementById('totalPotentialBenefitChart');
            ...
            if (chartData && chartCanvas) {
                ...
                // Add headers
                const headers = ['Perusahaan', 'Akumulasi Total Potensial Benefit'];
                ...
                a.href = url;
                a.download = `total_potential_benefit_company_${event_name}.xlsx`;
                a.click();
    ...
    if (exportPdfButton) {
        exportPdfButton.addEventListener('click', async function () {
            ...
            if (chartData && chartCanvas) {
                ...
                // Add title
                pdf.setFontSize(18);
                pdf.text(`Total Potensial Benefit Perusahaan`, 10, 10);

                // Add table headers
                pdf.setFontSize(12);
                let yPosition = 20;
                pdf.text('Perusahaan', 10, yPosition);
                pdf.text('Akumulasi Total Potensial Benefit', 100, yPosition);
                ...
                html2canvas(chartCanvas).then(canvas => {
                    ...
                    // Save the PDF
                    pdf.save(`total_potential_benefit_company_${event_name}.pdf`);
                });
            }
        });
    }
