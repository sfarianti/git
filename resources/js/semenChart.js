...
document.addEventListener("DOMContentLoaded", function () {
    const charts = JSON.parse(
        document.getElementById(
            "charts-data-persebaran-inovasi-setiap-perusahaan",
        ).textContent,
    );
    ...
    charts.forEach((chart, index) => {
        let ctx = document
            .getElementById("persebaran-inovasi-perusahaan-chart-" + index)
            .getContext("2d");
        new Chart(ctx, {
            type: "bar",
            data: {
                ...
                datasets: [
                    {
                        label: chart.company,
                        data: chart.data,
                        backgroundColor: chart.categories.map(
                            (cat) => categories[cat] ?? "#ffffff",
                        ),
                        borderWidth: 1,
                        barThickness: 50,
                    },
