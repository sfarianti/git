...
class GenerateTeamWithPaperWithoutEvent extends Command
{
    protected $signature = 'generate:team-paper-without-event';
    ...
    public function handle()
    {
        ...
        // Step 8: Simpan file paper ke dalam tabel papers
        $storagePath = "internal/papers/{$teamName}";
        Storage::makeDirectory($storagePath);

        $fullPaperPath = "{$storagePath}/full_paper.pdf";
        $localTestFullPaper = 'test-files/full-paper-test.pdf';

        if (Storage::disk('local')->exists($localTestFullPaper)) {
            Storage::disk('public')->put($fullPaperPath, Storage::disk('local')->get($localTestFullPaper));
        ...
        Paper::create([
            ...
            'full_paper' => Storage::disk('public')->exists($fullPaperPath) ? $fullPaperPath : null,
            'financial' => 1000000,
