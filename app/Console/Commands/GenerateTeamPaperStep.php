...
class GenerateTeamPaperStep extends Command
{
    protected $signature = 'generate:team-paper-step';
    ...
    public function handle()
    {
        ...
        $steps = explode(',', $randomMetodologi->step); // Misalnya langkah-langkah dipisahkan koma

        // Menambahkan nilai 'tes' pada setiap langkah
        $stepCount = count($steps);
        $stepsData = array_fill(0, $stepCount, 'tes'); // Isi dengan 'tes' sebanyak jumlah step

        Paper::create([
            'team_id' => $team->id,
            'step_1' => $stepsData[0] ?? '-', // Menggunakan nilai 'tes' atau '-' jika tidak ada
            'step_2' => $stepsData[1] ?? '-',
            'step_3' => $stepsData[2] ?? '-',
            'step_4' => $stepsData[3] ?? '-',
            'step_5' => $stepsData[4] ?? '-',
